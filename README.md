...
Meaning sits beside Parallel passages. It opens a short note on what that verse is saying in its place. The words are Matthew Henry’s Concise Commentary on the Whole Bible, which is in the public domain. A note is often written for a few verses together. The verse he tapped shows that note, and the screen says which verses it covers. When this commentary has no note for that verse, the screen says so. It does not write a new explanation. The note is in English. In Spanish and Portuguese the button, the empty state, and the line under the note follow his language, and a line says the explanation is in English. The source and license are in `public/scripture/meaning-license.txt`.
